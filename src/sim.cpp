...
void Sim::Run() {
	...
	while (this->win.isOpen()) {
		...
		{
			case TWO_FLUID:
			    if(this->container.GetDensityPointer()[IX(1,1,this->container.GetSize())] == 0) {
					this->container.AddDensity((SIZE/2), 1, 500);
					this->container.AddDensity((SIZE/2), SIZE-2, 500);
				}
				break;
			case ONE_FLUID_BASIC_CORNER:
				if(this->container.GetDensityPointer()[IX(1,1,this->container.GetSize())] == 0) this->container.AddDensity(SIZE/2, 1, 500);
				break;
		...
		switch (exampleType) 
		{
			case TWO_FLUID:
				if(this->container.GetDensityPointer()[IX(1,1,this->container.GetSize())] == 0){
					this->container.AddVelocity(SIZE/2, 2, 0, 10);
					this->container.AddVelocity(SIZE/2, SIZE-3, 0, -10);
				}
				break;
			case ONE_FLUID_BASIC_CORNER:
				if(this->container.GetDensityPointer()[IX(1,1,this->container.GetSize())] == 0) this->container.AddVelocity(SIZE/2, 2, 0, 10);
				break;
			case ONE_FLUID_BASIC_CENTER_ALLDIR:
				if(this->container.GetDensityPointer()[IX(1,1,this->container.GetSize())] == 0){
